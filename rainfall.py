# -*- coding: utf-8 -*-
"""
Created on Sun Mar 29 14:17:46 2020
...
fig = plt.figure(figsize=(18,10))
ax = fig.add_subplot(111)
df_res.plot.line(title='Different b/w Actual and Predicted (Training Data)',ax=ax, color = 'c',fontsize=20)
...
regressor.fit(df_train.drop('x4',axis=1),df_train['x4'])


y_pred = regressor.predict(df_train.drop('x4',axis=1))

residuals = y_pred-df_train['x4'].values
df_res = pd.DataFrame(residuals)
df_res.columns = ['Residuals']
print('MAD (Training Data): ' + str(np.mean(np.abs(residuals))))
